...
#line 61
  id<GsonInstanceCreator> typeCreator =
#line 62
  [((id<JavaUtilMap>) nil_chk(instanceCreators_)) getWithId:type];
  if (typeCreator != nil) {
    return new_GsonConstructorConstructor_1_initWithGsonInstanceCreator_withJavaLangReflectType_(typeCreator, type);
  }
  
#line 72
  id<GsonInstanceCreator> rawTypeCreator =
#line 74
  [instanceCreators_ getWithId:rawType];
...
#line 185
    if ([JavaUtilConcurrentConcurrentMap_class_() isAssignableFrom:rawType]) {
...
#line 198
    [((GsonTypeToken *) nil_chk(GsonTypeToken_getWithJavaLangReflectType_(IOSObjectArray_Get(nil_chk([((id<JavaLangReflectParameterizedType>) nil_chk(((id<JavaLangReflectParameterizedType>) cast_check(type, JavaLangReflectParameterizedType_class_())))) getActualTypeArguments]), 0)))) getRawType]])) {
